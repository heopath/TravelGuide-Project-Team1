...
async function run() {
  /* ── 마크업 ── */
  {
    ...
    T("수정·삭제 입력을 두지 않는다",
      !section.includes("<form") && !section.includes("data-audit-delete") && !section.includes("<select"));
  ...
  /* ── 전후 값 병합 ── */
  {
    ...
    T("바뀐 값은 이전 → 이후로 보여준다", lines.includes("status: DRAFT → SOLD_OUT"));
    T("한쪽에만 있는 값도 빠뜨리지 않는다", lines.includes("name: 모의 관광 티켓 20"));
  }
  ...
  /* ── 등록처럼 before가 없는 경우 ── */
  {
    ...
    T("before가 없으면 이후 값만 보여준다", changes(rows(d)[0]).includes("name: 새 상품"));
    T("등록 동작도 한국어로 보여준다",
