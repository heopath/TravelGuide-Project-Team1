...
document.addEventListener("DOMContentLoaded", function () {
  ...
  let proceededToRecommendations = false;

  window.addEventListener("pagehide", function () {
    if (proceededToRecommendations) return;
  ...
  async function saveTrip(draft){
    ...
    // API 준비 전에는 확정된 요청 JSON을 실제로 전송하지 않고 mock 저장으로 화면 흐름을 검증한다.
    if(window.ALL_MY_TRIPS_TRIP_API_READY!==true){
      await new Promise(function(resolve){window.setTimeout(resolve,500)});
      return {data:{mock:true,payload:payload,nextUrl:"/trips/recommendations"},message:"여행 정보를 임시 저장했습니다."};
    }
