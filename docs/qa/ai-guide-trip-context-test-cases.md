...
| AI-CONTEXT-06 | 본인 소유 `tripId`로 API 요청 | 기존 여행·DAY·사용자 선호가 Cohere 요청 프롬프트에 포함되고 추천 응답 반환 |
...
```powershell
.\gradlew.bat test --tests "org.example.all_my_trip_project.domain.ai.controller.AiGuideControllerTest" --tests "org.example.all_my_trip_project.domain.ai.service.AiGuideContextServiceTest" --tests "org.example.all_my_trip_project.domain.ai.service.AiGuideServiceTest" --tests "org.example.all_my_trip_project.domain.ai.service.CohereAiModelClientTest"
```

자동 테스트는 `tripId` 양의 정수 검증, 본인 여행·DAY·선호 컨텍스트 구성, 타인 여행 조회 실패 전파, Cohere 요청 프롬프트 반영을 확인한다. 화면 이동 버튼과 실제 Cohere 호출은 일정 화면 연결 후 수동으로 확인한다.
