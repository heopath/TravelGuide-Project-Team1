...
- [x] 오류 코드와 HTTP 상태 확정
- [x] 백엔드 컴파일·단위 배선 확인
- [ ] 페이지 담당자(남현호) 검수 — 8절 미확정 항목 확정(8절 1~3번은 실제로는 모두 관리자 화면·ADMIN 권한 범위라 담당 표기가 부정확해 보인다 — 아래 참고)
- [x] 관리자 화면 담당자(허민재) 검수 — 8-1번 ADMIN 권한 검사 이관 방식은 이슈 #144 댓글에서 확인함(관리자 계정 생성 방법 자체가 없다는 선행 이슈를 별도로 분리하겠다고 함). 8-2·8-3번(페이지네이션·기록 상태 연동)은 관리자 화면 설계 시 같이 정리하기로 함.
- [ ] `trips/record.html`(신고 버튼) 연동 — 보류. `trips/record.html`은 본인 소유 기록만 보여주는 화면이라 자기 기록을 신고하는 흐름이 성립하지 않는다는 의견을 이슈 #144에 남겼고, 팀 응답 대기 중이다. 공개 피드(다른 사용자의 기록 열람) 화면이 생기면 그때 신고 버튼을 붙이는 게 맞다고 보고 있다.
