...
### DB — 운영 상태와 화면 표현 데이터를 분리

`database/migration/V19__support_chat.sql`이 봇의 운영 상태를 이미 상정하고 있어 방 상태나
메시지 본문용 컬럼은 더 추가하지 않습니다.
...
4. 대화 기록과 운영 상태는 기존 정규화 테이블에 두고, 가변적인 화면 표현만 메시지 블록에 둔다.
...
**확정:** 고객센터 챗봇은 Gemini REST API를 사용하되, 여행 AI 가이드의 OpenAI REST 경로와 프롬프트를 재사용하지 않습니다. 두 기능은 목적과 응답 형식이 다르므로 **상담 전용 `SupportChatBotClient`와 별도 시스템 프롬프트**로 분리합니다(`domain.support` 아래). Spring AI `ChatModel`은 사용하지 않습니다.
...
**봇 → `WAITING` 전환 기준(2026-08-24 AI 문맥 판단으로 변경)**: 사용자가 상담원 연결을 명시적으로 요청했거나, 봇의 확인 질문에 동의한 경우에만 사람 대기로 넘어갑니다. 이 판단은 Java의 단어 목록이나 `contains` 검사로 하지 않습니다. Gemini가 전체 대화 문맥을 읽고 아래 세 결과 중 하나를 구조화된 표시로 반환하며, 서버는 결과를 실행만 합니다.

- `CONNECT` (`[HANDOFF]`): 명시적인 현재 연결 요청 또는 직전 확인 질문에 대한 긍정 답변 → `WAITING` 전환
- `CONFIRM` (`[CONFIRM_HANDOFF]`): 연결 의도가 불명확하거나 재확인이 필요한 상황 → 확인 질문만 표시하고 `BOT` 유지
- `NONE` (표시 없음): 일반 상담 응답 → `BOT` 유지

“상담원 연결하지 마”, “상담원 연결 그만 물어봐”, “왜 자꾸 상담원을 연결하냐” 같은 부정·거절·불만 문맥은 `NONE`으로 처리하도록 프롬프트에 명시했습니다. Gemini 호출 실패·시간 초과·사용량 한도 초과처럼 AI가 문맥을 판단하지 못한 경우에도 서버가 연결 의도를 추측하지 않습니다. 방을 `BOT`에 유지하고 AI 상담 이용 불가 안내만 표시합니다.
...
**연결돼 있지 않은 동안에만** 일정 주기(3초)로 REST를 대신 부릅니다 — 손님 화면은 열어 둔 방을, 관리자 화면은 상담 목록과 열어 둔 방을 함께 다시 읽습니다. 매 틱마다 연결 상태를 다시 확인하므로 연결되면 스스로 멈춥니다. WebSocket이 정상인 동안에는 이 폴링이 전혀 개입하지 않습니다.
...
**마이페이지·마이티 연결 통합 및 스크롤 위치 보존(2026-08-24 추가)**: 두 화면 모두 `support-chat-live.js` 공통 모듈로 방 토픽과 사용자 오류 큐를 구독한다. 마이티도 더 이상 3초 폴링을 기본 수신 방식으로 사용하지 않으며, WebSocket 연결 전·장애 중에만 공통 3초 폴백이 동작한다. 응답이나 이벤트로 대화 목록을 다시 그릴 때는 현재 위치와 하단까지의 거리를 저장하고, 하단에서 48px 이내를 보고 있던 경우에만 새 메시지를 따라간다. 사용자가 그보다 위를 읽고 있으면 기존 `scrollTop`을 복원한다.
...
방 생성 → BOT (봇이 자동 응답)
            ...
            │                          CLOSED ──▶ (새 방) BOT
            │
            └─ AI가 명시적 요청·확인 동의로 판단(CONNECT) ──▶ WAITING (사람 대기)
                        │                              │
...
1. **Gemini 실패만으로 `WAITING`으로 넘기거나 연결 여부를 묻지 않는다.** 일시적 실패는 방을 `BOT`에 둔 채 재시도 안내를 남긴다. 연속 실패, API 키 미설정, 사용량 한도 초과처럼 AI가 문맥을 판단할 수 없는 경우에는 AI 상담 이용 불가 안내만 남긴다. 상담원 연결은 정상적인 AI 응답이 `CONNECT`로 판단했을 때만 실행한다.
...
### 9. 손님용 API — 직접 상담원 연결 · 봇 복귀 · 새 상담 시작 (2026-08-24 추가)

손님이 AI 문장 판단에만 의존하지 않고 직접 상담원을 선택할 수 있는 경로와, "8. 상태 전이"의 갇힌 방 문제를 해결하는 경로를 제공한다. 모두 `SupportChatController`(`/api/v1/support/chat`) 아래에 있고, 응답 형태는 `SupportChatViewResponse`(방 + 메시지)를 그대로 돌려준다.

| 엔드포인트 | 조건 | 동작 |
| --- | --- | --- |
| `POST /api/v1/support/chat/request-agent` | 열린 방이 `BOT`일 때 | 사용자가 확인창에서 직접 선택한 요청이므로 Gemini를 호출하지 않고 방을 `WAITING`으로 바꾼다. 상태 변경 직전 방을 잠그고 다시 `BOT`인지 확인한다. |
...
두 화면(마이페이지 고객센터 탭, 화면 구석 마이티 위젯) 모두 `BOT`에서 사용자가 직접 상담원 연결을 선택할 수 있다. 마이페이지는 버튼을 바로 보여주고, 공간이 좁은 마이티는 입력창 왼쪽 `+` 버튼 안에 상담 옵션을 접어 둔다. 연결을 선택하면 실수 방지 확인창을 거친다. `WAITING`에서는 "봇에게 다시 물어보기"와 "새 상담 시작"을, `ASSIGNED`에서는 "새 상담 시작"만 제공한다. 마이티의 화면용 `보낼 말` 문구는 제거하되 보조기기용 라벨은 유지한다. `CLOSED`에서는 모든 상담 옵션을 숨긴다.
...
표시 순서 인덱스만 사용해 최근 메시지 목록을 한 번의 추가 조회로 채우며, 메시지마다 따로
조회하는 N+1 방식은 쓰지 않는다.

장소 추천은 RAG가 DB에서 찾은 실제 장소 후보를 Gemini에 전달하고, Gemini가 그 후보 번호만
선택하게 한다. 서버는 선택 번호가 후보에 포함됐는지와 현재 `places` 테이블에 존재하는지를
다시 확인한 뒤 이름·분류·주소·설명·이미지·평점·추천 이유를 화면용 스냅샷으로 저장한다.
클라이언트는 임의 URL을 읽지 않고 `/guide/places/{placeId}`를 직접 조합한다. RAG가 꺼져
있거나 검색이 실패하면 일반 텍스트 답변만 표시하고, 만들어 낸 장소 카드는 저장하지 않는다.

Gemini 할당량이 없는 로컬 환경에서는
`database/seed/local_seed_support_chat_blocks.sql`을 실행하면 테스트 사용자의 열린 상담방에
액션 버튼과 실제 `places` 행 3개를 사용한 장소 카드 메시지가 추가된다. 이 파일은 로컬
...
- 과거 액션 backfill은 한 트랜잭션의 `INSERT ... SELECT`다. 운영 메시지 수를 먼저 세고,
  ...
  인덱스는 만들지 않으며, 메시지 번호·표시 순서 인덱스만 사용한다.
- 로컬 seed 파일은 테스트 사용자 전용이다. 운영 DB에는 실행하지 않는다.
...
- [x] 마이페이지와 마이티가 공통 WebSocket 연결을 우선 사용하고, 연결 실패 중에만 3초 REST 폴백으로 방을 갱신하며 연결되면 폴백 조회를 건너뛴다
- [x] 고객이 위의 대화를 읽는 동안 폴링·WebSocket 재렌더링이 스크롤을 최하단으로 강제로 내리지 않는다. 원래 하단을 보고 있던 경우에만 새 메시지를 따라간다.
...
- [x] AI가 문맥상 명시적인 요청 또는 연결 확인 동의라고 판단하지 않으면 상담원 대기로 넘어가지 않고 방이 `BOT`에 유지된다. Gemini 실패 시에도 서버가 연결을 추측하지 않는다 — "설계 > 8. 상태 전이" 참고
- [x] 손님이 AI 대화 없이 `BOT` 상태에서 상담원 연결 버튼으로 직접 사람 응대를 요청할 수 있다 — "설계 > 9. 손님용 API" 참고
...
- [x] 운영 상태는 정규화 컬럼에 유지하고 화면 표현만 JSONB 메시지 블록에 저장한다 — "설계 > 11" 참고
- [x] AI 장소 추천은 DB/RAG 후보에 존재하는 장소만 카드로 표시하고 실제 장소 상세로 연결한다
