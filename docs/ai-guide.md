...
3. `모의 응답 상태`가 `성공 응답`이면 `docs/examples/ai-guide-response.json`의 구조로 추천과 출처가 표시됩니다.
4. `실패 응답`으로 바꾸면 오류 안내와 `다시 시도` 버튼이 표시됩니다. 성공 응답으로 바꾼 뒤 다시 시도하면 정상 결과를 확인할 수 있습니다.
5. 항공권·숙소 검색은 새 탭으로 `/booking/flights`, `/booking/hotels`를 엽니다.
...
응답 JSON은 `docs/examples/ai-guide-response.json`을 기준으로 합니다. `success`, `data`, `message` 공통 응답 형식과 DTO 필드가 최종 확정되면 `ai-guide.js`의 `requestMock`만 실제 `fetch` 호출로 교체합니다.
